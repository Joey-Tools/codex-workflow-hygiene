...
class OrchestratorTests(unittest.TestCase):
    ...
    def complete_shadow_partial(
        ...
        retained = coordinator.load_state()["retained_export"]
        bundle = (
            self.root
            / ".codex-local"
            / "shadow-successors"
    ...
    def test_high_risk_second_review_hash_adjudication_and_synthesis(self) -> None:
        ...
        retained = coordinator.store.load()["retained_export"]
        run_state = retained["run_state"]
        review_data = retained["review_data"]
    ...
    def test_formal_completion_closes_jobs_and_cleans_raw_working_data(
        ...
        agent_sinks = coordinator.run_dir / "agent-sinks"
        working_paths = [
            path
            for root in (raw_input, raw_shards, agent_sinks)
