...
- Pin the Codex root and every rollout ancestor with descriptor-relative no-follow directory opens, then open the enumerated final regular file with no-follow and nonblocking flags. Treat disappearance, symlink, non-directory ancestors, non-regular final entries, or identity changes before descriptor pinning as coverage errors instead of reopening by pathname or interpreting them as an originally missing root. Once an ancestor is pinned, later pathname replacement cannot redirect the held descriptor. For active `sessions/**` and legacy root session metadata, capture in-scope final-entry identity during the same `scandir` iteration and advance append-only baselines through enumeration, open, descriptor, and path checks: device/inode must stay fixed, size may not decrease, and an unchanged size requires an unchanged full snapshot. Keep every `archived_sessions/**` layout on exact full-snapshot identity.
