...
- Pin the Codex root and every rollout ancestor with descriptor-relative no-follow directory opens, then open the enumerated final regular file with no-follow and nonblocking flags. Treat disappearance, symlink, non-directory ancestors, non-regular final entries, or identity changes before descriptor pinning as coverage errors instead of reopening by pathname or interpreting them as an originally missing root. Once an ancestor is pinned, later pathname replacement cannot redirect the held descriptor. For session metadata, bind an enumerated final entry by regular-file device and inode so normal same-inode append remains valid while replacement still fails closed.
