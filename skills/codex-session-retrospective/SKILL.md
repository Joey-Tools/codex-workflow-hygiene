...
For local sources, `scan-*` and `discover` may automatically write bounded generated summaries under an ignored sibling `.codex-local/session-retrospective/*-generated-rollout-summaries/` directory and reference that root only from the transient manifest. Do not retain that directory or treat it as a source-of-truth copy of the raw rollout.
