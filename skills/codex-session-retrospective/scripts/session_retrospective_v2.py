...
def _embedded_retained_inputs(
    ...
    if not isinstance(run_state, Mapping) or not isinstance(review_data, Mapping):
        raise CliContractError(
            ...
            message="the retained export payload is incomplete",
        )
    return dict(run_state), dict(review_data)
...
def _retained_inputs(
    ...
    embedded = _embedded_retained_inputs(state)
    if embedded is None:
        raise CliContractError(
            exit_code=ExitCode.INVALID_STATE,
            code="retained_payload_unavailable",
            message="the v2 run lacks its deterministic retained projection",
        )
    return embedded


def _load_prior_period(path_value: str | None) -> dict[str, Any] | None:
...
def command_export(args: argparse.Namespace) -> CommandResult:
    ...
    run_state, review_data = _retained_inputs(state)
