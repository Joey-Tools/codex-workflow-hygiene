...
class RunLifecycleOperations(OrchestratorComponent):
    ...
    def _shadow_coverage_payload(
        ...
        *,
        ...
        retained = state.get("retained_export")
        if not isinstance(retained, Mapping):
            raise InvalidTransitionError("shadow run lacks retained export inputs")
        run_state = retained.get("run_state")
        review_data = retained.get("review_data")
        if not isinstance(run_state, Mapping) or not isinstance(review_data, Mapping):
            raise InvalidTransitionError("shadow retained export inputs are invalid")
        if (
    ...
    def _raw_cleanup_claim_value(
        ...
        *,
        ...
        body = {
            ...
            "raw_path_inventory": list(SHADOW_CLEANUP_ROOTS),
        ...
        return {
            **body,
            "claim_ref": "raw_cleanup_claim_v2:"
            + self.identity.derive_digest("raw-cleanup-claim-v2", body),
        }
    ...
    def _validate_raw_cleanup_claim(
        ...
        *,
        ...
        inventory = self._validated_cleanup_inventory(claim, label="raw")
    ...
    def _validated_cleanup_inventory(
        ...
        *,
        ...
        if (
            claim.get("raw_path_inventory") != list(SHADOW_CLEANUP_ROOTS)
            or not isinstance(counters, Mapping)
            or set(counters) != set(SHADOW_CLEANUP_ROOTS)
            or not isinstance(objects, Mapping)
            or set(objects) != set(SHADOW_CLEANUP_ROOTS)
        ):
            raise InvalidTransitionError(f"{label} cleanup inventory is invalid")
        for name in SHADOW_CLEANUP_ROOTS:
    ...
    def _validate_completed_raw_cleanup(
        ...
        *,
        ...
        self._validated_cleanup_inventory(claim, label="completed raw")
        unsigned = dict(claim)
        claim_ref = unsigned.pop("claim_ref", None)
        expected_ref = "raw_cleanup_claim_v2:" + self.identity.derive_digest(
            "raw-cleanup-claim-v2",
            unsigned,
        )
        ...
        if (
            claim.get("schema") != "raw_cleanup_claim_v2"
            or claim.get("run_ref") != state["run_ref"]
            or claim.get("deadline") != state["deadlines"]["raw"]
            or claim.get("disposition") != disposition
            or claim.get("raw_path_inventory") != list(SHADOW_CLEANUP_ROOTS)
        ):
    ...
    def _raw_cleanup_inventory(self) -> dict[str, Any]:
    ...
    def _shadow_cleanup_claim_value(
        ...
        return {
            **body,
            "claim_ref": "shadow_cleanup_claim_v2:"
            + self.identity.derive_digest("shadow-cleanup-claim-v2", body),
        }
    ...
    def _validate_shadow_cleanup_claim(
        ...
        if (
            claim["schema"] != "shadow_cleanup_claim_v2"
            or claim["run_ref"] != state["run_ref"]
            or claim["coverage_receipt_ref"] != coverage["receipt_ref"]
            or claim["export_bundle_digest"] != coverage["export_bundle_digest"]
            or claim["raw_path_inventory"] != list(SHADOW_CLEANUP_ROOTS)
            or not isinstance(counters, Mapping)
            or set(counters) != set(SHADOW_CLEANUP_ROOTS)
            or not isinstance(objects, Mapping)
            or set(objects) != set(SHADOW_CLEANUP_ROOTS)
    ...
    def complete_shadow_export(self) -> dict[str, Any]:
        ...
        if phase == "shadow_complete":
            self._validate_completed_shadow_cleanup(state)
            response = self._projection._status_view(snapshot)
            response.update(
                {
        ...
        def close(current: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
            ...
            coverage = self._validated_shadow_coverage(current)
            ...
            cleanup = self._authenticated_run_receipt(
                schema=authority.SHADOW_CLEANUP_RECEIPT_SCHEMA,
                ref_domain="raw_cleanup_receipt_v2",
                auth_domain="raw_cleanup_auth_v2",
                payload={
                    "bundle_digest": coverage["export_bundle_digest"],
