"""State queries, metrics, references, and materialized public action views."""
...
class StateProjectionOperations(OrchestratorComponent):
    ...
    def _source_transport_output_path(self, lease_ref: str) -> Path:
        return (
            self.run_dir
            / RAW_INPUT_DIRECTORY
            / "source-preparations"
    ...
    def _public_source_job(self, job: Mapping[str, Any]) -> dict[str, Any]:
        ...
        stream_path = self._source_transport_output_path(lease.lease_ref)
        expected_relative = stream_path.relative_to(self.run_dir).as_posix()
        if job.get("source_transport_output_relative") != expected_relative:
            raise InvalidTransitionError(
                "source transport output binding does not match its lease"
            )
        try:
            safe_io.check_owner_only_directory(stream_path.parent)
