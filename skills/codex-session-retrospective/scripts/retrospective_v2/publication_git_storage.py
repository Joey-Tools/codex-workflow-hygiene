...
class LocalGitStorageOperations:
    ...
    def _local_cleanup_claim(
        ...
        capacity = self._capacity_reservation(request.attempt_ref)
        ...
        *,
        ...
        for unit in units:
            ...
            if bundle_present and (sidecar.exists() or sidecar.is_symlink()):
                self._retained_export_lifecycle.release_staged_export(
                    bundle_dir,
                    request.attempt_ref,
                    disposition,
                )

    def _reserve_capacity(self, attempt_ref: str, amount: int) -> None:
        ledger = self._state_directory.read_json(self._capacity_path.name)
        _validate_capacity_ledger(ledger)
        reservations = dict(ledger["reservations"])
        existing = reservations.get(attempt_ref)
        if existing is not None:
            if existing != amount:
                raise StateCorruptionError("publication capacity reservation changed")
            return
        used = sum(int(value) for value in reservations.values())
        ...
        reservations[attempt_ref] = amount
        updated = dict(ledger)
        updated["reservations"] = reservations
        self._state_directory.write_json(self._capacity_path.name, updated)

    def _capacity_reservation(self, attempt_ref: str) -> int | None:
        with self._short_publication_lock():
            ledger = self._state_directory.read_json(self._capacity_path.name)
            _validate_capacity_ledger(ledger)
            value = ledger["reservations"].get(attempt_ref)
            return None if value is None else int(value)

    def _release_capacity(self, attempt_ref: str) -> None:
        with self._short_publication_lock():
            ledger = self._state_directory.read_json(self._capacity_path.name)
            _validate_capacity_ledger(ledger)
            reservations = dict(ledger["reservations"])
            if attempt_ref not in reservations:
                return
            del reservations[attempt_ref]
            updated = dict(ledger)
            updated["reservations"] = reservations
            self._state_directory.write_json(self._capacity_path.name, updated)
