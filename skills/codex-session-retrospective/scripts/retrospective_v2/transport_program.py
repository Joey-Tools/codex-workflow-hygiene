...
def _program_snapshot_python_flags() -> tuple[str, ...]:
    ...
    return _program_snapshot_protocol()._source_transport_snapshot_flags(
        ...
        cache=SOURCE_TRANSPORT_SNAPSHOT_CACHE,
...
class _ProgramSnapshotPythonFlags:
    def __iter__(self):
        return iter(_program_snapshot_python_flags())
...
def transport_program_commitment(command_argv: Sequence[str]) -> str:
    """Commit every executable component used by one source transport lease."""

    argv = tuple(command_argv)
    snapshot, worker_index, snapshot_commitment = _decode_program_snapshot(argv)
    ...
    if helper_count:
        ...
        if helper_count != 1 or helper_index + 1 >= len(argv):
            raise TransportValidationError(
        ...
        if not helper.is_absolute():
            raise TransportValidationError(
                "source transport remote helper path must be absolute"
            )
        components.append(
            _program_component(
                helper,
    ...
    return _canonical_commitment(
        {
            ...
            "schema": "source_transport_worker_program_v6",
        }
    )
