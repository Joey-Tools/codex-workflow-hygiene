...
_SHADOW_CLEANUP_CLAIM_RE = re.compile(r"shadow_cleanup_claim_v2:[0-9a-f]{64}\Z")
...
_RAW_CLEANUP_REF_RE = re.compile(r"raw_cleanup_receipt_v2:[0-9a-f]{64}\Z")
_RAW_CLEANUP_AUTH_RE = re.compile(r"raw_cleanup_auth_v2:[0-9a-f]{64}\Z")
...
SHADOW_CLEANUP_RECEIPT_SCHEMA = "raw_cleanup_receipt_v2"
_RAW_PATH_INVENTORY = ["raw-inputs", "raw-shards", "agent-sinks"]
_SOURCE_UNIT_FIELDS = {
    "consumed_candidate",
    "expected",
    "explicit_gap",
...
def _verify_shadow_cleanup_receipt(
    ...
    *,
    ...
    expected_ref = "raw_cleanup_receipt_v2:" + identity.derive_digest(
        "raw_cleanup_receipt_v2", body
    )
    expected_auth = "raw_cleanup_auth_v2:" + identity.derive_digest(
        "raw_cleanup_auth_v2", body
    )
    counts = (
    ...
    if (
        receipt["schema"] != SHADOW_CLEANUP_RECEIPT_SCHEMA
        or receipt["identity_key_id"] != identity.key_id
        or not isinstance(receipt["configuration_root"], str)
        or _DIGEST_RE.fullmatch(receipt["configuration_root"]) is None
        or not isinstance(receipt["production_configuration_ref"], str)
        ...
        or receipt["raw_path_inventory"] != _RAW_PATH_INVENTORY
        or any(
            not isinstance(item, int) or isinstance(item, bool) or item < 0
            for item in counts
        )
