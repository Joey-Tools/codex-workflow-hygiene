...
# inventory. These limits cover the three canonical hosts and four source kinds.
MAX_SOURCE_ACCEPTANCE_BYTES = 64 * 1024 * 1024
MAX_SOURCE_ACCEPTANCE_SEGMENTS_PER_CELL = 64
MAX_RUN_SOURCE_SEGMENTS = 3 * 4 * MAX_SOURCE_ACCEPTANCE_SEGMENTS_PER_CELL
MAX_RUN_SOURCE_SPOOL_LOCKS = MAX_RUN_SOURCE_SEGMENTS
MAX_RUN_SOURCE_RECORDS = 100_000
MAX_RUN_SOURCE_BYTES = 4 * 1024 * 1024 * 1024
MAX_RUN_SOURCE_ACCEPTANCE_BYTES = 256 * 1024 * 1024
MAX_RUN_AGENT_TASKS = 1_500
MAX_RUN_DOWNSTREAM_AGENT_TASKS = 500
MAX_RUN_RAW_SHARDS = MAX_RUN_AGENT_TASKS - MAX_RUN_DOWNSTREAM_AGENT_TASKS
MAX_AGENT_CHECKPOINT_TASK_BYTES = 16 * 1024
MAX_AGENT_ATTEMPTS_PER_TASK = 2
MAX_AGENT_CLAIM_GENERATIONS_PER_ATTEMPT = 2
MAX_AGENT_ARTIFACTS_PER_CLAIM = 2
MAX_AGENT_RUNTIME_FILES_PER_TASK = MAX_AGENT_ATTEMPTS_PER_TASK * (
    1 + MAX_AGENT_CLAIM_GENERATIONS_PER_ATTEMPT * MAX_AGENT_ARTIFACTS_PER_CLAIM
)
MAX_AGENT_CONTENT_SIDECARS_PER_TASK = 2  # Immutable task input plus accepted result.
MAX_RUN_DERIVED_SIDECARS = 1  # Reassembled extracted turns.
MAX_AGENT_ARTIFACTS_PER_TASK = (
    MAX_AGENT_RUNTIME_FILES_PER_TASK + MAX_AGENT_CONTENT_SIDECARS_PER_TASK
)
ATOMIC_CREATE_ENTRIES_PER_FILE = 2  # Target plus a conservative directory-lock share.
MAX_CLEANUP_FIXED_ENTRY_RESERVE = 20_000
MAX_CLEANUP_INVENTORY_ENTRIES = 300_000
MAX_CLEANUP_INVENTORY_PATH_BYTES = 64 * 1024 * 1024
MAX_CLEANUP_INVENTORY_BYTES = 256 * 1024 * 1024
MAX_CLEANUP_INVENTORY_DEPTH = 64
MAX_CLEANUP_INVENTORY_SECONDS = 300.0
MAX_CONSERVATIVE_RUN_CLEANUP_ENTRIES = (
    ATOMIC_CREATE_ENTRIES_PER_FILE
    * (
        MAX_RUN_AGENT_TASKS * MAX_AGENT_ARTIFACTS_PER_TASK
        + MAX_RUN_RAW_SHARDS
        + 1  # The raw-shard manifest.
        + MAX_RUN_SOURCE_RECORDS
        + MAX_RUN_SOURCE_SEGMENTS
        + MAX_RUN_DERIVED_SIDECARS
    )
    + MAX_RUN_SOURCE_SPOOL_LOCKS
    + MAX_CLEANUP_FIXED_ENTRY_RESERVE
)
if MAX_CONSERVATIVE_RUN_CLEANUP_ENTRIES > MAX_CLEANUP_INVENTORY_ENTRIES:
    raise RuntimeError("run artifact limits exceed cleanup inventory capacity")
...
def is_valid_session_identifier(value: object) -> bool:
    """Return whether a source identifier can be committed without ambiguity."""
    ...
    try:
        ...
        return False
    return len(encoded) <= MAX_SESSION_IDENTIFIER_BYTES and not any(
