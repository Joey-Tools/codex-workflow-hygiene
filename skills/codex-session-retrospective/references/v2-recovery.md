...
fd-anchored deletion of `raw-inputs`, `raw-shards`, and `agent-sinks`. Symlinks or
parent replacement fail closed and leave cleanup retryable. Successful expiry
removes source catalogs, payload bindings, jobs, shards, and retained working
results, preserving only authenticated cleanup and content-free gap/run metadata.
