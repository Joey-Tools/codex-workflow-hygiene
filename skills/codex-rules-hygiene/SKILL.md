...
4. Let the helper copy and validate the candidate inside an owner-private staging directory before it acquires the shared lock. Its validator supervisor requires a finite deadline, enforces an aggregate output-byte ceiling, and terminates and drains the validator process group on timeout, overflow, or surviving descendants. The helper revalidates identity, content, access policy, and link policy on the held candidate and live descriptors, plus file flags, xattrs, and ACL admission, after validation, immediately before exchange, and after installation. Under the lock it publishes the bound backup with an atomic no-replace rename, records the recovery receipt, and exchanges the private candidate with live rules. The helper supports these primitives on Darwin and Linux and fails closed without falling back when the C library, metadata API, or filesystem does not support them.
