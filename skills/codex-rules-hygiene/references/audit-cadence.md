...
- Disposable-stage cleanup runs only after the validator process group is quiescent and removes a leaf only while the held owner-private stage identity/access policy is unchanged and the leaf still names a transaction-bound object. Unknown or replacement entries are retained and reported. `recovery_required` skips disposable cleanup and retains descriptor-bound recovery evidence.
