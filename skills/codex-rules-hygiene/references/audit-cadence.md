...
- Candidate validity protects exact bytes and object policy: the helper copies the source into a descriptor-bound `0700` staging directory, creates an owner-only regular file with mode `0600` and `st_nlink == 1`, validates the held descriptor and leaf binding, then repeats the single-link and descriptor-backed metadata admission after the validator, during access-policy application, immediately before publication or exchange, and after installation. A hard link or unmodeled metadata added after the last preflight is still detected by the post-mutation descriptor and entry proofs and produces `recovery_required`.
...
- Exclusive-write failure and stage cleanup never unlink or remove a pathname after a comparison. Cleanup distinguishes a verified namespace link from an FD-only object and closes the latter without claiming that it survives process exit.
