...
6. The helper derives `rules/default.rules` and `.default.rules.apply.lock` from `CODEX_HOME`, with the ordinary Codex home as the fallback. Pass validator arguments directly; the helper replaces the one exact `'{rules}'` argument with first the private candidate path and then the live path. Do not wrap the validator in `bash -lc`. The timeout must be finite and positive. The supervisor captures at most 128 KiB across stdout and stderr and terminates, drains, and reaps the validator process group on timeout, output overflow, surviving descendants, cancellation, or supervisor observation failure. Before validator launch it blocks `SIGINT`, `SIGTERM`, and `SIGHUP`, installs a first-signal gate, lets the child inherit the launcher's original mask, and arms only after `Popen` returns a bound child PID/PGID. A managed signal then closes the gate against later interruption, completes TERM/KILL/drain/reap, restores the original handlers and mask, and returns `128 + signal`.
7. Accept only `applied` or `no_change_after_lock` as successful apply outcomes. Apparent no-change still takes the shared lock and admits the live file's complete metadata and single-link policy, but it returns before private staging or validator launch. Only a proved uncertain namespace mutation or `recovery_required` deliberately selects descriptor-bound recovery-evidence retention; the mere existence of an evidence object does not. POSIX has no compare-and-unlink or compare-and-rmdir primitive, so disposable cleanup never runs a check-then-delete sequence. It atomically moves the complete owner-private stage to a fresh no-clobber `.default.rules.cleanup-retained-*` quarantine, binds the moved directory identity, and emits `stage_cleanup_retained` with its locator. A racing replacement may be moved and reported but is never deleted. This verified cleanup warning does not change an already proved `applied` or rolled-back result; `stage_cleanup_refused` is processed before terminal output and overrides any pending success. `post_replace_failed_rolled_back` means the requested cleanup did not apply. `recovery_required` means an atomic publication or exchange outcome, or the resulting protected properties, could not be proved. Preserve the emitted backup, schema-v2 receipt, identity-bound recovery-copy locators, and object observations for operator-directed recovery. Treat `retention_incomplete` as proof that one or more descriptor-only objects could not be copied durably—not as a retained object.
...
- Candidate validity protects exact authorized bytes and object policy: the helper requires a candidate SHA-256 independent of the audited live digest, checks it before copying, creates an owner-only regular file with mode `0600` and `st_nlink == 1` inside a descriptor-bound `0700` stage, validates the held descriptor and leaf binding, and then revalidates the original candidate-source identity, content, access policy, and digest after private validation. It repeats the single-link and descriptor-backed metadata admission during access-policy application, immediately before publication or exchange, and after installation. A hard link or unmodeled metadata added after the last preflight is still detected by the post-mutation descriptor and entry proofs and produces `recovery_required`.
...
- Disposable-stage cleanup runs only after the validator process group is quiescent. It does not use `stat` followed by `unlink`, an empty-directory check followed by `rmdir`, or any other pathname delete that could remove a last-moment replacement. Instead it uses atomic no-clobber rename to quarantine the whole stage, then verifies the moved directory against the descriptor-bound identity and access policy. A proved quarantine is a reported cleanup warning, not recovery evidence; an unsupported, failed, replaced, or unverified quarantine is `stage_cleanup_refused` and cannot accompany a success result. `recovery_required` skips disposable quarantine and retains descriptor-bound recovery evidence.
- Automatic rollback and `recover` both require live rules to match the receipt's installed protected properties and require the backup to match its bound protected properties. Successful rollback records and revalidates a sibling owner-private recovery-terminal identity. A later `already_original` result requires the receipt's exact original identity or that exact terminal evidence; same-content replacement inodes are refused. Any uncertain exchange requires operator recovery rather than a second destructive guess.
