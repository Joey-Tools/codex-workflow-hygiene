...
- Candidate validity protects exact bytes and object policy: the helper copies the source into a descriptor-bound `0700` staging directory, creates an owner-only regular file with mode `0600` and `st_nlink == 1`, validates the held descriptor and leaf binding, then repeats the single-link check after the validator, during access-policy application, and immediately before publication or exchange. A hard link added after the last preflight is still detected by the post-mutation descriptor and entry proofs and produces `recovery_required`.
...
- Schema-v2 receipts bind those regular-file properties and the rules-parent object identity; every original, installed, and backup snapshot must record `nlink: 1` or recovery refuses the receipt. Recovery remains compatible with schema-v1 receipts, whose missing link policy is treated as unknown rather than invented. The helper refuses nonzero file flags or extended attributes because it cannot preserve them; callers must separately prove that platform ACLs are absent when ACL state is not exposed through those checks.
...
- A recovery locator is emitted only after the held rules-parent FD is rebound to the configured pathname, the discovered stage/file entries are identity-checked again, and the parent is rebound a second time. If the parent path was replaced, an entry was concurrently unlinked, or a recovery copy could not be completed, the locator is null and the result says `retention_incomplete` or `descriptor_only_or_unlocatable`.
...
- Automatic rollback and `recover` both require live rules to match the receipt's installed protected properties and require the backup to match its bound protected properties. Any uncertain exchange requires operator recovery rather than a second destructive guess.
