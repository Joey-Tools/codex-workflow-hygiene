...
- Collapse a second file only when it is byte-identical or its normalized prefix contains already-accounted-for assistant or tool-execution history. For prefix comparison, ignore only known direct `session_meta` runtime-context drift and canonicalize known generated item, call, response, and turn IDs by per-rollout order while preserving their reference relationships. Keep lifecycle/provenance IDs, unknown record fields, and nested IDs or timestamps inside substantive content and tool results. Preserve an uncertain prompt-only match even under one session identity; identical wrappers plus a repeated user prompt are not sufficient replay evidence. If either copy has a distinct suffix, retain that suffix, especially later genuine human follow-ups.
