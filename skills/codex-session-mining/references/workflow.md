...
- Collapse a second file only when it is byte-identical or its normalized prefix contains already-accounted-for assistant or tool-execution history. Preserve an uncertain prompt-only match even under one session identity; identical wrappers plus a repeated user prompt are not sufficient replay evidence. If either copy has a distinct suffix, retain that suffix, especially later genuine human follow-ups.
