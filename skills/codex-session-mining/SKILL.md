...
- If the corpus-helper deadline expires or the run is interrupted, classify the result as incomplete. Before any retry, prove the original producer and its descendants are quiescent; if that cannot be proved, stop and hand off the evidence without starting another full-root scan. After quiescence is proved, retry with a different fresh nonexistent output directory; do not remove or reuse the failed-run path until its recorded directory object identity is revalidated. If the target is missing at revalidation, do not recreate it or delete anything at that pathname; retain the recorded path and identity as unresolved handoff evidence. If quiescence becomes uncertain, the baseline identity is unavailable, or the current identity cannot be proved, leave any present path untouched and retain the evidence for handoff instead of reporting a completed scan or verified cleanup.
