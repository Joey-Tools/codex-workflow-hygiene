...
- If the corpus-helper deadline expires or the run is interrupted, classify the result as incomplete. Retry with a different fresh nonexistent output directory; do not remove or reuse the failed-run path until the producer and descendants are proven quiescent and its recorded directory object identity is revalidated. If the target is missing at revalidation, do not recreate it or delete anything at that pathname; retain the recorded path and identity as unresolved handoff evidence. If quiescence, the baseline identity, or the current identity cannot be proved, leave any present path untouched and retain the evidence for handoff instead of reporting a completed scan or verified cleanup.
