...
- Prefer the default `evidence` mode. Use `user-text` only when the task specifically needs structurally identified user text, and use repeatable category filters to narrow evidence without searching arbitrary values.
...
- When the first result window is insufficient, continue with `--result-offset` and the prior `frozen_prefix_bytes`, or raise the bounded result limits once. Each invocation independently rescans a descriptor-bound prefix; multiple windows are not a content-stable snapshot.
