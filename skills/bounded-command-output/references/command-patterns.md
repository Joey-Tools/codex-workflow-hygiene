...
The wrapper runs direct argv without an implicit shell, inherits standard I/O, and normally adds only one process plus process-group setup. It installs cancellation handlers before `Popen`; a signal received during process startup is recorded and forwarded as soon as the process-group handoff completes. On timeout it sends `TERM` to the process group, waits the complete grace period without reaping the leader so the PGID cannot be reused, sends best-effort `KILL`, and waits only for its direct child. A normal child exit is returned unchanged; a deadline returns `124`, and an externally received `INT`, `TERM`, or `HUP` is forwarded to the group before the wrapper returns the conventional `128 + signal` status.
