...
- `search` defaults to a typed `evidence` mapping, offers a conservative structural `user-text` mode, and does not expose an `all-values` mode. Evidence matches retain category, role, field path, and bounded snippets; `custom_tool_call.input` and custom tool output are explicit mappings. User-message matches also preserve a bounded direct `origin_hint` as record-level provenance without making it searchable or treating role=`user` as human.
...
- Metadata and registered lifecycle `event_msg` families are opt-in through explicit categories. The event mapping covers `task_started`, abort, stream-retry/error, and review-mode transitions without promoting unknown event payloads into arbitrary-value evidence. Results retain physical order, support category filters, and use stateless result windows rather than a snapshot cursor.
- The default result window is 20 records and 64 KiB of serialized result-event bytes; the legal byte-budget range is 512 bytes through 256 KiB. When the first full result cannot fit an otherwise empty byte window, a fixed-size compact match preserves its physical coordinates, matched categories, observed-hit count, and full-event byte requirement while explicitly marking the omitted details. This guarantees strict offset progress without weakening the selected-field or input-coverage contract. Every invocation remains an independent descriptor-prefix observation.
...
- Complete records advance through each read buffer with an offset and one batch compaction rather than deleting and shifting the buffer for every short record; physical byte coordinates, read accounting, record limits, and deferred-tail semantics remain unchanged.
...
- Focused scanner and structure validation: `python3 -B -m unittest -q tests.test_scan_rollout tests.test_skill_structure` (`73` tests, passed).
- Full repository validation: `python3 -B -m unittest discover -s tests -b -q` (`255` tests, passed).
- Python 3.10 baseline scanner validation: `/Users/hoteng/.local/share/uv/python/cpython-3.10-macos-aarch64-none/bin/python3.10 -B -m unittest -q tests.test_scan_rollout` (`65` tests, passed); CI retains the required `test` context and adds a separate `python-310` scanner job.
...
- The final PR-head local gate review found that a present non-string `payload.type` incorrectly fell back to the outer evidence type. The scanner now reserves outer-type fallback for a missing discriminator, with null, list, object, and numeric regression cases.
- Current-head GitHub Codex review `4947176306` found three P2 gaps: registered event-message families were not searchable, per-record buffer deletion repeated short-record shifts, and user matches omitted `origin_hint` provenance. The implementation now has explicit opt-in event mappings, batch buffer compaction with deterministic complexity tests, and bounded record-level origin projection; the resulting new head requires fresh local and GitHub review gates.
- Current-head GitHub Codex review `4947286003` found that the legal 512-byte result budget could suppress the first match and return the unchanged result offset indefinitely. The scanner now emits an explicitly truncated compact first match under detail pressure, keeps later-page boundaries contiguous, and guarantees that every non-null continuation offset advances; the resulting new head requires fresh local and GitHub review gates.
- The fresh local review at `084cd55b23c53f90e3cdc13f84eb7e61eec6caad` found that the 980-level deep-walk fixture exhausted the supported Python 3.10 JSON decoder before exercising the scanner. The end-to-end case now uses a parse-safe depth, a separate already-decoded tree exceeds the interpreter recursion limit to prove iterative evidence traversal, and CI exercises the scanner on Python 3.10; the resulting new head requires fresh gates.
