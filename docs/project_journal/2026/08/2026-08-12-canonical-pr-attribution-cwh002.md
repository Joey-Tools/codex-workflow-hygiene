...
- `python3 -B -m unittest tests.test_pr_attribution`: 74 tests passed.
- Full repository suite: 1,133 tests passed with commit signing disabled only for temporary Git fixtures.
...
- Current-head GitHub Codex identified three additional gaps. The follow-up now establishes every rollout content digest before inventory publication, canonicalizes `model`/`model_id` provider aliases before replay comparison and voting, and rejects non-printable traversed directory components during both enumeration and projection revalidation.
