...
- The field-aware fallback decodes each physical record as strict UTF-8 before JSON parsing, preventing `json.loads()` from auto-detecting UTF-16 or UTF-32 records and bypassing `invalid_records` accounting. Its narrow BOM-aware framing recovery assigns an aligned UTF-16LE/UTF-32LE encoded-LF trailer to the rejected foreign record so a following UTF-8 record remains independently searchable without stripping arbitrary NULs.
...
- Strict-decoding follow-up validation passed 59 focused contract/structure tests and the full 1,164-test repository suite with the CI ripgrep gate enabled; skill validation, project-journal validation, Python compilation, and `git diff --check` also passed.
