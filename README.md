...
The sole supported installed v2 coordinator is
`~/.codex/skills/codex-session-retrospective/scripts/session_retrospective_v2.py`.
It emits one canonical JSON result per command and exposes only deterministic
coordination actions; the automation runtime owns native subagent execution.

```bash
python3 "$HOME/.codex/skills/codex-session-retrospective/scripts/session_retrospective_v2.py" --help
```

See `skills/codex-session-retrospective/references/v2-cli.md` for the closed CLI
loop, `v2-engine-architecture.md` for deterministic module boundaries, and
`v2-shadow-cutover.md` for the non-publishing shadow gate. The v1 helper below
remains migration-only until v2 cutover and baseline succeed.

## Legacy Retrospective Baseline Dry Run
...
## Legacy Retrospective Weekly Dry Run
